%% ===== measureDlpGammaManual =====
% SELF-PACED version of measureDlpGamma.m for MANUAL radiometer ranging:
...
% advance to the next level. Nothing is timed; the sweep moves only when you
% hit Enter.
%
% Use measureDlpGamma.m instead when auto-range works (timed 5 s steps).
%
% WHAT IT DOES
...
%   4. Steps a full-screen solid color through
%          1.5625  3.125  6.25  12.5  25  50  75  100   percent
%      LOW -> HIGH, then the same list back high -> low (every level shown
%      twice; the top level twice back-to-back at the turnaround). Each level
%      HOLDS until you press Enter. For the red channel the codes sent are
%      (4,0,0), (8,0,0), (16,0,0), ... (255,0,0), then back down.
%   5. Before the final step (the second pass at 1.5625 %) it parks the DLP
%      at (0,0,0) again so you can RE-ZERO the meter after the bright half of
%      the sweep (rezero_before_steps adds holds before other steps too).
...
% i.e. the second pass at 1.5625 %. Add indices to hold before more of the
% dim tail, e.g. nSteps-2:nSteps. [] = no holds.
...
fprintf('[measureDlpGammaManual] Connected to Stage at %s. Canvas: %d x %d\n', stageHost(), W, H);
