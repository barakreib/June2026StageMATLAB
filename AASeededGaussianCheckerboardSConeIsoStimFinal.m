...
% tables for Stage serialization compatibility. No local subfunctions.
    ...
    % ---- Write stimulus log CSV (one row per cell per update: linear + adjusted) ----
    % S-iso: R=v, G=1-v, B=0 per cell. Per-channel correction keeps EMITTED
    % R_light+G_light = 1 (codes deliberately do not sum to 255).
    logData = zeros(nUpdates * checksY * checksX, 10);
    idx = 0;
    for u = 1:nUpdates
        startFrame = (u - 1) * updateEveryNFrames;   % 0-based frame this pattern starts
        t          = startFrame / refreshRate;
        V    = noiseVals(:, :, u);
        Radj = lcGammaCorrect(V);
        Gadj = lcGammaCorrect(1 - V);
        for r = 1:checksY
            for c = 1:checksX
                v = V(r, c);
                idx = idx + 1;
                logData(idx, :) = [startFrame, t, r, c, v, 1 - v, 0, Radj(r, c), Gadj(r, c), 0];
            end
        end
    end
    logHeader = {'frame', 'time_s', 'checkRow', 'checkCol', 'R', 'G', 'B', 'R_adj', 'G_adj', 'B_adj'};
    logMeta = { ...
        ...
        'row_cadence',           'per_update'; ...
        'created',               datestr(now, 'yyyy-mm-dd HH:MM:SS'); ...
        'data_start_row',        0 };
    logPath = writeStimLog(pwd, logMeta, logHeader, logData);
    fprintf('[AASeededGaussianCheckerboardSConeIsoStimFinal] Wrote stimulus log: %s\n', logPath);
