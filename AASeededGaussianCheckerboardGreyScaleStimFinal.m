...
% tables for Stage serialization compatibility. No local subfunctions.
    ...
    % real experiments -- long checkerboard runs make large files.
    if loadRigConfig('debug_values_csv', false)
        writeStimValuesCsv(pwd, 'AASeededGaussianCheckerboardGreyScaleStimFinal', seed, struct('grey', noiseVals));
    end
    ...
    % (checks_y x checks_x x n_updates) noise from `seed` via reproduce_noise, and
    % the sent values via gamma_adjust. Rows pair to Clampex .abf files by order.
    record = struct( ...
