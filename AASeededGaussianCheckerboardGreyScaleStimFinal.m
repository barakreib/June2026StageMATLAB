...
% tables for Stage serialization compatibility. No local subfunctions.
    ...
    for u = 1:nUpdates
        ...
        V    = noiseVals(:, :, u);
        Vadj = lcGammaCorrect(V);
        for r = 1:checksY
            for c = 1:checksX
                v = V(r, c);  a = Vadj(r, c);
                idx = idx + 1;
                logData(idx, :) = [startFrame, t, r, c, v, v, v, a, a, a];
            end
        end
    end
    logHeader = {'frame', 'time_s', 'checkRow', 'checkCol', 'R', 'G', 'B', 'R_adj', 'G_adj', 'B_adj'};
    logMeta = { ...
        'stimulus',              'AASeededGaussianCheckerboardGreyScaleStimFinal'; ...
    ...
    fprintf('[AASeededGaussianCheckerboardGreyScaleStimFinal] Wrote stimulus log: %s\n', logPath);
